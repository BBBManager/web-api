...
class IMDT_Util_Date {
    ...
    public static function diffExt($startSeconds, $endSeconds = null) {
        ...
        if ($endSeconds <= $startSeconds)
            return 'Cálculo de datas inválido';
        ...
        $arrFinal = array();
        if ($diff_weeks > 0)
            $arrFinal[] = $diff_weeks . (($diff_weeks > 1) ? 'w' : 'w');
        if ($diff_days > 0)
            $arrFinal[] = $diff_days . (($diff_days > 1) ? 'd' : 'd');
        if ($diff_hours > 0)
            $arrFinal[] = $diff_hours . (($diff_hours > 1) ? 'hrs' : 'h');
        if ($diff_minutes > 0)
            $arrFinal[] = $diff_minutes . (($diff_minutes > 1) ? 'mins' : 'm');
        if ($diff_seconds > 0)
            $arrFinal[] = $diff_seconds . (($diff_seconds > 1) ? 'secs' : 's');
    ...
    public static function diff($startSeconds, $endSeconds = null) {
        ...
        if ($diff_days > 0)
            $arrFinal[] = $diff_days . (($diff_days > 1) ? 'd' : 'd');
        if ($diff_hours > 0)
            $arrFinal[] = $diff_hours . (($diff_hours > 1) ? 'h' : 'h');
        if ($diff_minutes > 0)
            $arrFinal[] = $diff_minutes . (($diff_minutes > 1) ? 'min' : 'min');
        if ($diff_seconds > 0)
            $arrFinal[] = $diff_seconds . (($diff_seconds > 1) ? 's' : 's');
