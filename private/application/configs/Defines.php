...
final class BBBManager_Config_Defines
{
    ...
    public static $ACL_ACCESS_PROFILES_UPDATE_RESOURCE = 'access-profiles-update';
    public static $ACL_ROOM_USERS_IMPORT_RESOURCE = 'room-users-import';
    ...
    public static function getAclResources()
    {
        return array(
            ...
            self::$ACL_ACCESS_PROFILES_UPDATE_RESOURCE,
            self::$ACL_ROOM_USERS_IMPORT_RESOURCE
        );
    }
    ...
    public static function getAccessProfile($accessProfileId = null)
    {
        $rAccessProfile = array(
            ...
            null => IMDT_Util_Translate::_('Inherited from Group'),
        );

        if ($rAccessProfile == null) {
    ...
    public static function getMemberRoomProfile($memberRoomProfileId = null, $plural = false)
    {

        if ($plural == true) {
            $rMemberRoomProfile = array(
                self::$ROOM_ADMINISTRATOR_PROFILE => IMDT_Util_Translate::_('Administrators'),
                self::$ROOM_MODERATOR_PROFILE => IMDT_Util_Translate::_('Moderators'),
                self::$ROOM_PRESENTER_PROFILE => IMDT_Util_Translate::_('Presenters'),
                self::$ROOM_ATTENDEE_PROFILE => IMDT_Util_Translate::_('Attendees')
            );
        } else {
            $rMemberRoomProfile = array(
                self::$ROOM_ADMINISTRATOR_PROFILE => IMDT_Util_Translate::_('Administrator'),
                self::$ROOM_MODERATOR_PROFILE => IMDT_Util_Translate::_('Moderator'),
                self::$ROOM_PRESENTER_PROFILE => IMDT_Util_Translate::_('Presenter'),
                self::$ROOM_ATTENDEE_PROFILE => IMDT_Util_Translate::_('Attendee')
            );
        }

        if ($memberRoomProfileId == null) {
            return $rMemberRoomProfile;
        } else {
            return $rMemberRoomProfile[$memberRoomProfileId];
        }
    }
