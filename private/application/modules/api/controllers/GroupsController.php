...
class Api_GroupsController extends Zend_Rest_Controller{
    ...
    public function init() {
        ...
        $this->select = $this->model->select()
                                    ...
                                    ->joinLeft('user_group','user_group.group_id = g.group_id',
                                                array('user_attendee' => new Zend_Db_Expr('GROUP_CONCAT(distinct user_group.user_id SEPARATOR ",")')))
                                    //->joinLeft('user','user.user_id = user_group.user_id',array())
                                    ->joinLeft('group_group','group_group.parent_group_id = g.group_id',
                                                array('group_attendee_local' => new Zend_Db_Expr('GROUP_CONCAT(distinct case when group_group.auth_mode_id = 1 then group_group.group_id else null end SEPARATOR ",")'),
    ...
    public function deleteAction() {
        try {
            if(strpos($this->_id, ',') == -1) {
                ...
                if($rowModel == null) throw new Exception(sprintf($this->_helper->translate('Group %s not found.'), $this->_id));
    ...
    public function putAction() {
        ...
        try {
            ...
            $rowModel = $this->model->find($this->_id)->current();
            if($rowModel == null) throw new Exception($this->_helper->translate('Group %s not found.'));
