...
class Api_MaintenanceController extends Zend_Rest_Controller {
    ...
    public function init() {
        ...
        $this->model = new BBBManager_Model_Maintenance();

        $this->columnValidators = array();
        $this->columnValidators['active'] = array(new Zend_Validate_NotEmpty());

        $this->filters['active'] = array('column' => 'active', 'type' => 'integer');
        $this->acessLog();
    }
    ...
    public function acessLog() {
        ...
        if ($this->_getParam('id', false)) {
            ...
            if ($this->view->response['success'] == '1') {
                ...
                $desc = $this->view->response['row']['maintenance_id'];
            }
    ...
    public function deleteAction() {
        try {
            if (strpos($this->_id, ',') == -1) {
                ...
                if ($rowModel == null)
                    throw new Exception(sprintf($this->_helper->translate('Maintenance %s not found.'), $this->_id));

                $this->view->response = array('success' => '1', 'msg' => $this->_helper->translate('The maintenance was deleted successfully.'));
                $rowModel->delete();
            } else {
                ...
                $this->view->response = array('success' => '1', 'msg' => sprintf($this->_helper->translate('%s maintenances was deleted successfully.'), $i));
            }
        } catch (Exception $e) {
    ...
    public function indexAction() {
        try {
            ...
            $this->view->response = array('success' => '1', 'collection' => $rCollection, 'teste' => $authData, 'msg' => sprintf($this->_helper->translate('%s maintenances retrieved successfully.'), count($rCollection)));
        } catch (Exception $e) {
            $this->view->response = array('success' => '0', 'msg' => $e->getMessage());
        }
    }
    ...
    public function postAction() {
        ...
        try {
            ...
            $authorizationHash = BBBManager_Util_MaintenanceMode::generateAuthorizationHash($rowModel);

            if (isset($rowValidColumns['authorization_hash'])) {
                $rowModel->authorization_hash = $authorizationHash;
            }
            ...
            $this->view->response = array('success' => '1', 'msg' => '', 'id' => $newRowId, 'msg' => $this->_helper->translate('Maintenance has been created successfully.'));
        } catch (Exception $e) {
            $this->view->response = array('success' => '0', 'msg' => $e->getMessage());
        }
    }
    ...
    public function putAction() {
        ...
        try {
            ...
            $rowModel = $this->model->find($this->_id)->current();
            if ($rowModel == null)
                throw new Exception($this->_helper->translate('Maintenance %s not found.'));
