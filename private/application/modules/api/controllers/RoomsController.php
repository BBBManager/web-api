...
class Api_RoomsController extends Zend_Rest_Controller {
    ...
    public function init() {
        ...
        $this->select = $this->model->select()
                ...
                ->joinLeft(array('mrc' => 'meeting_room_category'), 'mrc.meeting_room_category_id = mr.meeting_room_category_id', array(
                    'meeting_room_category_name' => 'mrc.name'
                        )
                )
                ->joinLeft(array('mrg' => 'meeting_room_group'), 'mrg.meeting_room_id = mr.meeting_room_id', array(
                ...
                ->joinLeft(array('mru' => 'meeting_room_user'), 'mru.meeting_room_id = mr.meeting_room_id', array(
                ...
                ->joinLeft(
                    ...
                    'recordings' => new Zend_Db_Expr(
                        ...
                        ), 'recordings.meeting_room_id = mr.meeting_room_id', array(
                    'has_recordings'
                        )
                )
    ...
    public function putAction() {
        ...
        try {
            ...
            if (!isset($data['date_start']))
                unset($this->columnValidators['date_start']);
            if (!isset($data['date_end']))
                unset($this->columnValidators['date_end']);
            if (!isset($data['name']))
                unset($this->columnValidators['name']);
            if (!isset($data['participants_limit']))
                unset($this->columnValidators['participants_limit']);
            if (!isset($data['url']))
                unset($this->columnValidators['url']);
            ...
            $rowModel = $this->model->find($this->_id)->current();
            if ($rowModel == null)
                throw new Exception($this->_helper->translate('Meeting Room %s not found.'));
