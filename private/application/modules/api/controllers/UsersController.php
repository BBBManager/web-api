...
class Api_UsersController extends Zend_Rest_Controller {
	...
	try {
		...
		if ($rowModel == null) throw new Exception(sprintf($this->_helper->translate('User %s not found.'), $this->_id));
        
        if($isPrivilegedUser && $rowModel->access_profile_id != BBBManager_Config_Defines::$SYSTEM_USER_PROFILE) throw new Exception('No permission to perform this action.');
		...
		foreach ($collection as $row) {
		    if($isPrivilegedUser && $row->access_profile_id != BBBManager_Config_Defines::$SYSTEM_USER_PROFILE) throw new Exception('No permission to perform this action.');
		    $row->delete();
		    $i++;
		}
	...
	try {
	    ...
	    if (count($rowModel) == 0) throw new Exception(sprintf($this->_helper->translate('User %s not found.'), $this->_id));
        
        if(IMDT_Util_Auth::getInstance()->get('access_profile_id') != BBBManager_Config_Defines::$SYSTEM_ADMINISTRATOR_PROFILE 
                && IMDT_Util_Auth::getInstance()->get('access_profile_id') != BBBManager_Config_Defines::$SYSTEM_SUPPORT_PROFILE
                && IMDT_Util_Auth::getInstance()->get('access_profile_id') != BBBManager_Config_Defines::$SYSTEM_PRIVILEGED_USER_PROFILE
                &&  IMDT_Util_Auth::getInstance()->get('id') != $this->_id) {
	...
	try {
        ...
        if($data['auth_mode_id'] == BBBManager_Config_Defines::$LOCAL_AUTH_MODE) {
            ...
            if(!empty($newUserPrefix)) $data['login'] = $newUserPrefix.$data['login'];
        }
	...
	try {
        ...
        //Privileged User validations
        if(IMDT_Util_Auth::getInstance()->get('access_profile_id') == BBBManager_Config_Defines::$SYSTEM_PRIVILEGED_USER_PROFILE) {
            if(array_key_exists('login', $data)) unset($data['login']);
            if(array_key_exists('auth_mode_id', $data)) unset($data['auth_mode_id']);
            if(array_key_exists('access_profile_id', $data)) unset($data['access_profile_id']);
            if(array_key_exists('groups', $data)) unset($data['groups']);
        }
	    ...
	    $rowModel = $this->model->find($this->_id)->current();
	    if ($rowModel == null) throw new Exception($this->_helper->translate('User %s not found.'));
